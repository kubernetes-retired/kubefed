...
type FederatedTypeConfigInformer interface {
	...
	Lister() internalversion.FederatedTypeConfigLister
}
...
func NewFederatedTypeConfigInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredFederatedTypeConfigInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Federation().FederatedTypeConfigs().List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Federation().FederatedTypeConfigs().Watch(options)
			},
		},
		&federation.FederatedTypeConfig{},
		resyncPeriod,
		indexers,
	)
}

func (f *federatedTypeConfigInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *federatedTypeConfigInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&federation.FederatedTypeConfig{}, f.defaultInformer)
}

func (f *federatedTypeConfigInformer) Lister() internalversion.FederatedTypeConfigLister {
	return internalversion.NewFederatedTypeConfigLister(f.Informer().GetIndexer())
}
