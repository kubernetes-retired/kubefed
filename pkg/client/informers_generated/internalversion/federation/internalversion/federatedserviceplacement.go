...
type FederatedServicePlacementInformer interface {
	...
	Lister() internalversion.FederatedServicePlacementLister
}
...
func NewFederatedServicePlacementInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredFederatedServicePlacementInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Federation().FederatedServicePlacements(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Federation().FederatedServicePlacements(namespace).Watch(options)
			},
		},
		&federation.FederatedServicePlacement{},
		resyncPeriod,
		indexers,
	)
}

func (f *federatedServicePlacementInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *federatedServicePlacementInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&federation.FederatedServicePlacement{}, f.defaultInformer)
}

func (f *federatedServicePlacementInformer) Lister() internalversion.FederatedServicePlacementLister {
	return internalversion.NewFederatedServicePlacementLister(f.Informer().GetIndexer())
}
