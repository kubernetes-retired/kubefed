...
type FederatedConfigMapInformer interface {
	...
	Lister() internalversion.FederatedConfigMapLister
}
...
func NewFederatedConfigMapInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredFederatedConfigMapInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Federation().FederatedConfigMaps(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Federation().FederatedConfigMaps(namespace).Watch(options)
			},
		},
		&federation.FederatedConfigMap{},
		resyncPeriod,
		indexers,
	)
}

func (f *federatedConfigMapInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *federatedConfigMapInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&federation.FederatedConfigMap{}, f.defaultInformer)
}

func (f *federatedConfigMapInformer) Lister() internalversion.FederatedConfigMapLister {
	return internalversion.NewFederatedConfigMapLister(f.Informer().GetIndexer())
}
