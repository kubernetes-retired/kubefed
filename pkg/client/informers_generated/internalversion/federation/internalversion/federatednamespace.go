...
type FederatedNamespaceInformer interface {
	...
	Lister() internalversion.FederatedNamespaceLister
}
...
func NewFederatedNamespaceInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredFederatedNamespaceInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Federation().FederatedNamespaces().List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Federation().FederatedNamespaces().Watch(options)
			},
		},
		&federation.FederatedNamespace{},
		resyncPeriod,
		indexers,
	)
}

func (f *federatedNamespaceInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *federatedNamespaceInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&federation.FederatedNamespace{}, f.defaultInformer)
}

func (f *federatedNamespaceInformer) Lister() internalversion.FederatedNamespaceLister {
	return internalversion.NewFederatedNamespaceLister(f.Informer().GetIndexer())
}
