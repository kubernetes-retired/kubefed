...
type FederatedReplicaSetInformer interface {
	...
	Lister() internalversion.FederatedReplicaSetLister
}
...
func NewFederatedReplicaSetInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredFederatedReplicaSetInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Federation().FederatedReplicaSets(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Federation().FederatedReplicaSets(namespace).Watch(options)
			},
		},
		&federation.FederatedReplicaSet{},
		resyncPeriod,
		indexers,
	)
}

func (f *federatedReplicaSetInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *federatedReplicaSetInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&federation.FederatedReplicaSet{}, f.defaultInformer)
}

func (f *federatedReplicaSetInformer) Lister() internalversion.FederatedReplicaSetLister {
	return internalversion.NewFederatedReplicaSetLister(f.Informer().GetIndexer())
}
