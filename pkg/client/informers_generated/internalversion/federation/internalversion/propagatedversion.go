...
type PropagatedVersionInformer interface {
	...
	Lister() internalversion.PropagatedVersionLister
}
...
func NewPropagatedVersionInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredPropagatedVersionInformer(client internalclientset.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Federation().PropagatedVersions(namespace).List(options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Federation().PropagatedVersions(namespace).Watch(options)
			},
		},
		&federation.PropagatedVersion{},
		resyncPeriod,
		indexers,
	)
}

func (f *propagatedVersionInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *propagatedVersionInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&federation.PropagatedVersion{}, f.defaultInformer)
}

func (f *propagatedVersionInformer) Lister() internalversion.PropagatedVersionLister {
	return internalversion.NewPropagatedVersionLister(f.Informer().GetIndexer())
}
