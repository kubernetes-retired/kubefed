...
type FederatedDeploymentOverrideLister interface {
	...
	List(selector labels.Selector) (ret []*federation.FederatedDeploymentOverride, err error)
...
// List lists all FederatedDeploymentOverrides in the indexer.
func (s *federatedDeploymentOverrideLister) List(selector labels.Selector) (ret []*federation.FederatedDeploymentOverride, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*federation.FederatedDeploymentOverride))
	})
	return ret, err
}
...
type FederatedDeploymentOverrideNamespaceLister interface {
	...
	Get(name string) (*federation.FederatedDeploymentOverride, error)
...
// List lists all FederatedDeploymentOverrides in the indexer for a given namespace.
func (s federatedDeploymentOverrideNamespaceLister) List(selector labels.Selector) (ret []*federation.FederatedDeploymentOverride, err error) {
...
// Get retrieves the FederatedDeploymentOverride from the indexer for a given namespace and name.
func (s federatedDeploymentOverrideNamespaceLister) Get(name string) (*federation.FederatedDeploymentOverride, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(federation.Resource("federateddeploymentoverride"), name)
	}
	return obj.(*federation.FederatedDeploymentOverride), nil
}
