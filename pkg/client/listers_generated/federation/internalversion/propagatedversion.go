...
type PropagatedVersionLister interface {
	...
	List(selector labels.Selector) (ret []*federation.PropagatedVersion, err error)
...
// List lists all PropagatedVersions in the indexer.
func (s *propagatedVersionLister) List(selector labels.Selector) (ret []*federation.PropagatedVersion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*federation.PropagatedVersion))
	})
	return ret, err
}
...
type PropagatedVersionNamespaceLister interface {
	...
	Get(name string) (*federation.PropagatedVersion, error)
...
// List lists all PropagatedVersions in the indexer for a given namespace.
func (s propagatedVersionNamespaceLister) List(selector labels.Selector) (ret []*federation.PropagatedVersion, err error) {
...
// Get retrieves the PropagatedVersion from the indexer for a given namespace and name.
func (s propagatedVersionNamespaceLister) Get(name string) (*federation.PropagatedVersion, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(federation.Resource("propagatedversion"), name)
	}
	return obj.(*federation.PropagatedVersion), nil
}
