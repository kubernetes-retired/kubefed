...
type FederatedConfigMapLister interface {
	...
	List(selector labels.Selector) (ret []*federation.FederatedConfigMap, err error)
...
// List lists all FederatedConfigMaps in the indexer.
func (s *federatedConfigMapLister) List(selector labels.Selector) (ret []*federation.FederatedConfigMap, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*federation.FederatedConfigMap))
	})
	return ret, err
}
...
type FederatedConfigMapNamespaceLister interface {
	...
	Get(name string) (*federation.FederatedConfigMap, error)
...
// List lists all FederatedConfigMaps in the indexer for a given namespace.
func (s federatedConfigMapNamespaceLister) List(selector labels.Selector) (ret []*federation.FederatedConfigMap, err error) {
...
// Get retrieves the FederatedConfigMap from the indexer for a given namespace and name.
func (s federatedConfigMapNamespaceLister) Get(name string) (*federation.FederatedConfigMap, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(federation.Resource("federatedconfigmap"), name)
	}
	return obj.(*federation.FederatedConfigMap), nil
}
