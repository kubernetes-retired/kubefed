...
// List lists all FederatedSecrets in the indexer.
func (s *federatedSecretLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedSecret, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.FederatedSecret))
	})
	return ret, err
}
...
// List lists all FederatedSecrets in the indexer for a given namespace.
func (s federatedSecretNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedSecret, err error) {
...
// Get retrieves the FederatedSecret from the indexer for a given namespace and name.
func (s federatedSecretNamespaceLister) Get(name string) (*v1alpha1.FederatedSecret, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("federatedsecret"), name)
	}
	return obj.(*v1alpha1.FederatedSecret), nil
}
