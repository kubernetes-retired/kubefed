...
// List lists all FederatedServices in the indexer.
func (s *federatedServiceLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.FederatedService))
	})
	return ret, err
}
...
// List lists all FederatedServices in the indexer for a given namespace.
func (s federatedServiceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedService, err error) {
...
// Get retrieves the FederatedService from the indexer for a given namespace and name.
func (s federatedServiceNamespaceLister) Get(name string) (*v1alpha1.FederatedService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("federatedservice"), name)
	}
	return obj.(*v1alpha1.FederatedService), nil
}
