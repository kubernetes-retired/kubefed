...
// List lists all FederatedJobOverrides in the indexer.
func (s *federatedJobOverrideLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedJobOverride, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.FederatedJobOverride))
	})
	return ret, err
}
...
// List lists all FederatedJobOverrides in the indexer for a given namespace.
func (s federatedJobOverrideNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.FederatedJobOverride, err error) {
...
// Get retrieves the FederatedJobOverride from the indexer for a given namespace and name.
func (s federatedJobOverrideNamespaceLister) Get(name string) (*v1alpha1.FederatedJobOverride, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("federatedjoboverride"), name)
	}
	return obj.(*v1alpha1.FederatedJobOverride), nil
}
