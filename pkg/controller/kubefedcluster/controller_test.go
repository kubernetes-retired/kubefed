...
var _ = Describe("TestKubefedClusterController", func() {
	...
	DescribeTable(
		"threshold checked cluster status",
		func(clusterStatus *fedv1b1.KubeFedClusterStatus,
			storedClusterData *ClusterData,
			expectedClusterStatus *fedv1b1.KubeFedClusterStatus,
			expectedResultRun int64) {
			newClusterStatus := thresholdAdjustedClusterStatus(clusterStatus, storedClusterData, config)
			//Expect(expectedClusterStatus).To(Equal(newClusterStatus))

			//Expect(expectedResultRun).To(Equal(storedClusterData.resultRun))
			Expect(expectedClusterStatus).To(
				Equal(newClusterStatus),
				"Unexpected state, expected: %v, got:%v",
				expectedClusterStatus,
				newClusterStatus)

			Expect(expectedResultRun).To(
				Equal(storedClusterData.resultRun),
				"Unexpected resultRun, expected: %v, got:%v",
				expectedResultRun,
				storedClusterData.resultRun)
		},
		Entry("ClusterReadyAtBegining",
			clusterStatus(corev1.ConditionTrue, t1, t1),
			&ClusterData{clusterStatus: nil, resultRun: 0},
			clusterStatus(corev1.ConditionTrue, t1, t1),
			int64(1),
		),
		Entry("ClusterNotReadyAtBegining",
			clusterStatus(corev1.ConditionFalse, t1, t1),
			&ClusterData{clusterStatus: nil, resultRun: 0},
			clusterStatus(corev1.ConditionFalse, t1, t1),
			int64(1),
		),
		Entry("ClusterNotReadyButWithinFailureThreshold",
			clusterStatus(corev1.ConditionFalse, t3, t3),
			&ClusterData{
				clusterStatus: clusterStatus(corev1.ConditionTrue, t2, t1),
				resultRun:     2},
			clusterStatus(corev1.ConditionTrue, t3, t1),
			int64(3),
		),
		Entry("ClusterNotReadyAndCrossedFailureThreshold",
			clusterStatus(corev1.ConditionFalse, t4, t4),
			&ClusterData{
		...
		Entry("ClusterReturnToReadyState",
			clusterStatus(corev1.ConditionTrue, t5, t5),
			&ClusterData{
