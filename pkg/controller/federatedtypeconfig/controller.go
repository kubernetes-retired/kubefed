...
func (c *Controller) reconcile(qualifiedName util.QualifiedName) util.ReconciliationStatus {
	...
	if limitedScope && syncEnabled && !typeConfig.GetNamespaced() {
		glog.V(3).Infof("Skipping start of sync & status controller for cluster-scoped resource %q.  It is not required for a namespaced federation control plane.", typeConfig.GetFederatedType().Kind)
