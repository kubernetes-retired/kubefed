...
// +kubebuilder:controller:group=core,version=v1alpha1,kind=FederatedTypeConfig,resource=federatedtypeconfigs
...
func ProvideController(arguments args.InjectArgs, stopChan <-chan struct{}) (*controller.GenericController, error) {
...
func (c *FederatedTypeConfigController) startController(tc *corev1a1.FederatedTypeConfig) error {
	...
	err := synccontroller.StartFederationSyncController(tc, c.config, stopChan, false)
	if err != nil {
		close(stopChan)
		return fmt.Errorf("Error starting sync controller for %q: %v", tc.Spec.Template.Kind, err)
	}
	c.lock.Lock()
	defer c.lock.Unlock()
