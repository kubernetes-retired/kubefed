...
// EDIT THIS FILE!
// Created by "kubebuilder create resource" for you to implement the FederatedServiceStatus resource tests

var _ = Describe("FederatedServiceStatus", func() {
	var instance FederatedServiceStatus
	var expected FederatedServiceStatus
	var client FederatedServiceStatusInterface

	BeforeEach(func() {
		instance = FederatedServiceStatus{}
	...
	Describe("when sending a storage request", func() {
		Context("for a valid config", func() {
			It("should provide CRUD access to the object", func() {
				client = cs.CoreV1alpha1().FederatedServiceStatuses("default")
				...
				Expect(actual.ClusterStatus).To(Equal(expected.ClusterStatus))

				By("returning the item for list requests")
				result, err := client.List(metav1.ListOptions{})
				Expect(err).ShouldNot(HaveOccurred())
				Expect(result.Items).To(HaveLen(1))
				Expect(result.Items[0].ClusterStatus).To(Equal(expected.ClusterStatus))
