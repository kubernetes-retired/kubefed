...
// Package registrytestutil contains utilities for testing the etcd-based
// Cluster storage implementation.
package registrytestutil
...
func NewEtcdStorage(t *testing.T, group string) (*storagebackend.Config, *etcdtesting.EtcdTestServer) {
	server, config := etcdtesting.NewUnsecuredEtcd3TestClientServer(t, install.Scheme)
	config.Codec = install.Codecs.LegacyCodec(clusterregistry.SchemeGroupVersion)
...
func New(t *testing.T, storage *genericregistry.Store) *Tester {
	return &Tester{
		tester:  resttest.New(t, storage, install.Scheme),
		storage: storage,
	}
}
